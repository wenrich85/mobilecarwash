defmodule MobileCarWash.Notifications.BookingConfirmationWorkerTest do
  use MobileCarWash.DataCase, async: true
  use Oban.Testing, repo: MobileCarWash.Repo

  alias MobileCarWash.Notifications.BookingConfirmationWorker
  ...
  setup do
    ...
    %{appointment: appointment, customer: customer, service_type: service_type, address: address}
  end

  test "sends confirmation email for valid appointment", %{appointment: appointment} do
    assert :ok =
             perform_job(BookingConfirmationWorker, %{
  ...
  test "enqueues correctly" do
    assert {:ok, _job} =
             ...
             |> BookingConfirmationWorker.new(queue: :notifications)
             |> Oban.insert()
  end
end
