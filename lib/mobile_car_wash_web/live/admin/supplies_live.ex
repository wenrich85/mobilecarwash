defmodule MobileCarWashWeb.Admin.SuppliesLive do
  ...
  def handle_event("edit_supply", %{"id" => id}, socket) do
    ...
    form_data = %{
      ...
      "unit" => supply.unit,
      "low_stock_threshold" =>
        (supply.low_stock_threshold && Decimal.to_string(supply.low_stock_threshold)) || "",
      "unit_cost_cents" =>
        (supply.unit_cost_cents && div(supply.unit_cost_cents, 100) |> to_string()) || "",
  ...
  def handle_event(
        "save_restock",
        %{"qty" => qty_str, "cost" => cost_str, "notes" => notes},
        socket
      ) do
      ...
      <div :if={MapSet.size(@low_stock_ids) > 0} class="alert alert-warning mb-6">
        <div>
          <p class="font-semibold">
            ⚠ Low stock: {MapSet.size(@low_stock_ids)} item(s) need restocking
          </p>
          <p class="text-sm">
            {Enum.filter(@supplies, &MapSet.member?(@low_stock_ids, &1.id))
            |> Enum.map(& &1.name)
            |> Enum.join(", ")}
          </p>
        </div>
      ...
      <div :if={@editing} class="card bg-base-100 shadow mb-6">
        <div class="card-body">
          <h3 class="font-semibold mb-4">
            {if @editing == :new, do: "Add Supply", else: "Edit Supply"}
          </h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Amazon, local store…"
                />
              </div>
          ...
          <div class="card bg-base-100 shadow">
            <div class="overflow-x-auto">
              <table class="table table-sm">
                ...
                <tbody>
                  <tr
                    :for={supply <- cat_supplies}
                    class={[!supply.active && "opacity-40"]}
                  >
                    <td>
                      <div class="flex items-center gap-2">
                        <span class={["font-medium", !supply.active && "line-through"]}>
                          {supply.name}
                        </span>
                        <span
                          :if={MapSet.member?(@low_stock_ids, supply.id)}
                      ...
                      <div :if={supply.notes} class="text-xs text-base-content/70">
                        {supply.notes}
                      </div>
                    </td>
                    <td class={[
            ...
            <tbody>
              <tr :for={rec <- @usage_records}>
                ...
                <td class="font-semibold">
                  {format_qty(rec.quantity_used)} {viewed_supply && viewed_supply.unit}
                </td>
  ...
  defp parse_decimal(str, field) do
    case Decimal.parse(String.trim(str || "")) do
      {d, ""} ->
        if Decimal.gt?(d, Decimal.new(0)),
          do: {:ok, d},
          else: {:error, "#{field} must be a positive number"}
