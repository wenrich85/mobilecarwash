defmodule MobileCarWashWeb.Admin.SettingsLive do
  ...
  def handle_event("switch_tab", %{"tab" => tab}, socket) do
    {:noreply,
     assign(socket, tab: String.to_existing_atom(tab), editing_service: nil, editing_plan: nil)}
  end
  ...
  def handle_event("add_service", %{"service" => params}, socket) do
    ...
    case ServiceType |> Ash.Changeset.for_create(:create, attrs) |> Ash.create() do
      {:ok, _} ->
        ...
        {:noreply,
         socket |> assign(services: load_services()) |> put_flash(:info, "Service added")}
      ...
      {:ok, service} ->
        ...
        case service |> Ash.Changeset.for_update(:update, attrs) |> Ash.update() do
          {:ok, _} ->
            ...
            {:noreply,
             socket
             |> assign(services: load_services(), editing_service: nil)
      ...
      {:ok, plan} ->
        ...
        case plan |> Ash.Changeset.for_update(:update, attrs) |> Ash.update() do
          {:ok, _} ->
            ...
            {:noreply,
             socket
             |> assign(plans: load_plans(), editing_plan: nil)
  ...
  def render(assigns) do
    ...
    <!-- Tabs -->
      <div class="tabs tabs-boxed mb-8">
        ...
        <button
          class={["tab", @tab == :plans && "tab-active"]}
          phx-click="switch_tab"
          phx-value-tab="plans"
        >
          Membership Plans
        ...
        <button
          class={["tab", @tab == :blocked_dates && "tab-active"]}
          phx-click="switch_tab"
          phx-value-tab="blocked_dates"
        >
          Blocked Dates
        </button>
        <button
          class={["tab", @tab == :scheduling && "tab-active"]}
          phx-click="switch_tab"
          phx-value-tab="scheduling"
        >
          Scheduling
        </button>
        <button
          class={["tab", @tab == :accounting && "tab-active"]}
          phx-click="switch_tab"
    ...
    <!-- Service List -->
        <div class="space-y-3">
          <div
            :for={svc <- @services}
            class={["card bg-base-100 shadow-sm", !svc.active && "opacity-50"]}
          >
            <div class="card-body p-4">
              <!-- View mode -->
              <div :if={@editing_service != svc.id} class="flex justify-between items-center">
                ...
                <div class="flex gap-2">
                  <button class="btn btn-ghost btn-xs" phx-click="edit_service" phx-value-id={svc.id}>
                    Edit
                  </button>
                  <button
                    class={["btn btn-xs", if(svc.active, do: "btn-warning", else: "btn-success")]}
              ...
              >
                ...
                <div class="form-control">
                  ...
                  <input
                    ...
                    name="service[price]"
                    class="input input-bordered input-sm"
                    value={div(svc.base_price_cents, 100)}
                    required
                    step="1"
                    min="1"
                  />
    ...
    <!-- Plan List -->
        <div class="space-y-3">
          <div
            :for={plan <- @plans}
            class={["card bg-base-100 shadow-sm", !plan.active && "opacity-50"]}
          >
            <div class="card-body p-4">
              <!-- View mode -->
              <div :if={@editing_plan != plan.id}>
                <div class="flex justify-between items-start">
                  <div>
                    ...
                    <p class="text-2xl font-bold mt-1">
                      ${div(plan.price_cents, 100)}<span class="text-sm font-normal text-base-content/70">/month</span>
                    </p>
                  </div>
                  <div class="flex gap-2">
                    <button class="btn btn-ghost btn-xs" phx-click="edit_plan" phx-value-id={plan.id}>
                      Edit
                    </button>
                    <button
                      class={["btn btn-xs", if(plan.active, do: "btn-warning", else: "btn-success")]}
                ...
                <div class="flex flex-wrap gap-3 mt-2 text-sm">
                  <span :if={plan.basic_washes_per_month > 0} class="badge badge-outline">
                    {plan.basic_washes_per_month} basic washes
                  </span>
                  <span :if={plan.deep_cleans_per_month > 0} class="badge badge-outline">
                    {plan.deep_cleans_per_month} deep cleans
                  </span>
                  <span :if={plan.deep_clean_discount_percent > 0} class="badge badge-outline">
                    {plan.deep_clean_discount_percent}% off deep cleans
                  </span>
                </div>
                <p :if={plan.description} class="text-sm text-base-content/80 mt-2">
                  {plan.description}
                </p>
              </div>
              ...
              >
                ...
                <div class="form-control">
                  ...
                  <input
                    ...
                    name="plan[price]"
                    class="input input-bordered input-sm"
                    value={div(plan.price_cents, 100)}
                    required
                    step="1"
                    min="1"
                  />
                ...
                <div class="form-control">
                  ...
                  <input
                    ...
                    name="plan[basic_washes]"
                    class="input input-bordered input-sm"
                    value={plan.basic_washes_per_month}
                    min="0"
                  />
                </div>
                <div class="form-control">
                  ...
                  <input
                    ...
                    name="plan[deep_cleans]"
                    class="input input-bordered input-sm"
                    value={plan.deep_cleans_per_month}
                    min="0"
                  />
                </div>
                <div class="form-control">
                  ...
                  <input
                    ...
                    name="plan[discount]"
                    class="input input-bordered input-sm"
                    value={plan.deep_clean_discount_percent}
                    min="0"
                    max="100"
                  />
                </div>
              ...
              <div class="space-y-3 text-sm">
                ...
                <div :if={is_quickbooks_configured()} class="border-t border-info/30 pt-3">
                  <p class="font-mono font-semibold">
                    QuickBooks Online (when ACCOUNTING_PROVIDER=quickbooks):
                  </p>
                  <ul class="text-base-content/70 ml-4 mt-1 space-y-1">
  ...
  defp is_zoho_configured do
    Application.get_env(:mobile_car_wash, :accounting_provider) ==
